...
export class PanelMenus {
    ...
    private _search: SearchIndicatorView | null = null;
    ...
    private _recreate(): void {
        ...
        if (this._mode === 'menus' && this._settings.get_boolean('show-places-menu')) {
            this._places = new PlacesIndicator(this._settings);
            Main.panel.addToStatusArea(
                'sheliak-places', this._places.button, position++, box);
        }
        if (this._mode === 'menus' && this._settings.get_boolean('show-system-menu')) {
            this._system = new SystemIndicator(this._settings);
            Main.panel.addToStatusArea(
                'sheliak-system', this._system.button, position++, box);
        }
        if (this._mode === 'search' && this._createSearch && this._settings.get_boolean('show-search-menu')) {
