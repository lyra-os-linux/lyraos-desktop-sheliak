"""Run topbar allocation checks in a disposable GNOME 48 compositor."""
import argparse
import json
import os
...
if not args.inside_private_bus:
    with tempfile.TemporaryDirectory(prefix='sheliak-topbar-') as temporary:
        root = Path(temporary)
        env = os.environ.copy()
        ...
        (probe / 'metadata.json').write_text(json.dumps({'uuid': uuid, 'name': 'Topbar regression',
        ...
        apps.mkdir()
        (apps / 'org.lyraos.TopbarProbe.desktop').write_text(
            '[Desktop Entry]\nType=Application\nName=Lyra Topbar Probe\n'
            'Exec=/usr/bin/touch '+str(root / 'launched')+'\nIcon=applications-system\nTerminal=false\n')
        env['SHELIAK_LAUNCH_MARKER'] = str(root / 'launched')
...
assert os.environ['XDG_RUNTIME_DIR'].startswith('/tmp/sheliak-topbar-')
...
with (output / 'shell.log').open('w') as log:
    ...
    try:
        ...
        while time.monotonic() < deadline:
            ...
            time.sleep(.2)
        else: raise TimeoutError('Topbar checks did not finish')
